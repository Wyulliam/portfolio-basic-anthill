...
{
    public class TurboIncubatorTests
    {
        ...
        public TurboIncubatorTests()
        {
            _eggGenerator = Substitute.For<IEggGenerator>();
            _eggGetter = Substitute.For<IEggGetter>();
            _mediator = Substitute.For<IMediator>();
            _incubator = new TurboIncubator(_eggGenerator, _eggGetter, _mediator);
        }

        [Fact]
        public void should_incubate_eggs_in_2_seconds()
        {
            _incubator.SecondsToIncubate.Should().Be(2);
        }
